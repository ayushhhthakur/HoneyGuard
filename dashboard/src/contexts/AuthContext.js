...
import http, { setActiveOrgId, getActiveOrgId } from '../lib/http'
...
export const AuthProvider = ({ children }) => {
  ...
  const loadMe = useCallback(async (preferredOrgId) => {
    try {
      ...
      const stored = preferredOrgId || localStorage.getItem(ACTIVE_ORG_STORAGE_KEY)
  ...
  const createOrganization = useCallback(
    async (name) => {
      const { data } = await http.post(`${API_URL}/orgs`, { name })
  ...
  const acceptInvite = useCallback(
    async (token) => {
      const { data } = await http.post(`${API_URL}/orgs/invites/accept`, { token })
      await loadMe(data.data.id)
      return data.data
    },
    [loadMe],
  ...
  const value = useMemo(
    ...
    [session, profile, organizations, activeOrg, loading, error, login, signup, logout, resetPassword, switchOrg, createOrganization, acceptInvite, loadMe],
  )
